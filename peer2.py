...
class Echo(object):
    ...
    def push(self,chunk_id,chunk_data):
        infile = open('fichero_torrent2.txt', 'r+')
        for line in infile:
            word = MutableString(line)
        infile.close()
        if (word[chunk_id] == '0'):
            infile = open('fichero_torrent2.txt', 'w+')
            word[chunk_id]=chunk_data
            infile.write(str(word))
            infile.close()
            self.chunk_number += 1
            infile = open('average_peer2.txt', 'a')
    ...
    def start(self):
        ...
        infile.close()
        host_ref = "http://127.0.0.1:1295/peer2;Echo;peer2"
...
if __name__ == "__main__":
    ...
    host = create_host('http://127.0.0.1:1295/')
    peer1 = host.spawn('peer2', Echo)
    peer1.start()
    serve_forever()
